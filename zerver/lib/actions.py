...
def do_start_email_change_process(user_profile, new_email):
    ...
    from_email = "Zulip Account Security <%s>" % (settings.DEFAULT_FROM_EMAIL,)
    send_email('zerver/emails/confirm_new_email', new_email, from_email, context=context)
...
def do_send_confirmation_email(invitee, referrer, body):
    ...
    from_email = "%s (via Zulip) <%s>" % (referrer.full_name, settings.DEFAULT_FROM_EMAIL)
