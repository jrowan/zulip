...
def build_email(template_prefix, to_email, from_name=None, from_address=None, reply_to_email=None, context={}):
    ...
    # We check for from_name == "" becacuse in forms.py we overwrite the Django send_mail function
    # since we are subclassing the existing Django password reset form, and thus we need to be able
    # to handle cases where an address of the form "From Name <address@example.com>" is expected.
