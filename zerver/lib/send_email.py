...
def build_email(template_prefix, to_email, from_name=None, from_address=None, reply_to_email=None, context={}):
    ...
    (default_name, default_email) = parseaddr(settings.NOREPLY_EMAIL_ADDRESS)
    # If no from_name is given but settings.NOREPLY_EMAIL_ADDRESS specifies one,
    # e.g. 'Acme Corp. Zulip <zulip-noreply@acme.com>', use the name from settings.
